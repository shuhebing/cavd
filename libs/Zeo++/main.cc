...
//    exit(0);
  ...
  if(!error) {
    ...
    if(!error) {
      ...
      else if (strcmp(extension, "lmps") == 0){
        if(!readLMPSFile(name, &atmnet, radial)) error=true;
      }
            ...
            if(visualize){  // -vpsd or -zpsd, visualization run
              ...
              string filename_radii;
                            ...
                            if (visVisITflag){
                               ...
                               calcPoreSizeDistr(&atmnet, &orgAtomnet, highAccuracy, chan_radius, probe_radius, numSamples, false, filename_histogram, filename_points, filename_radii, "", visualize, visVisITflag);
                               }
                            if (!visVisITflag){
                               // here goes ZeoVis
                               }
              ...
              calcPoreSizeDistr(&atmnet, &orgAtomnet, highAccuracy,chan_radius, probe_radius, numSamples, false, filename_histogram, "", "", "", false, false);
            }
            //output.close();
            //outfile.close();
            //nodesRadii.close();
