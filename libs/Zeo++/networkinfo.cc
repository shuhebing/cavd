...
std::string lookupAtomName(double mass){
  
   mass = round(mass * 1000) / 1000; 
//  map <string,double>::iterator  info = massTable.find(mass);
  std::map<string,double>::iterator info;

  for (info=massTable.begin(); info!=massTable.end(); ++info)
    {
    if (info->second == mass) return info->first;
    };

  //if(info == massTable.end() ){
  //  cerr << "Unable to find atom name for mass " << mass << ". Please provide it " << "\n"
  //       << "Exiting ..." << "\n";
  //  exit(1);
  //};
