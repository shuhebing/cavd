...
/**
 * if use -migrant arguments, programm will read migrant ion element.
 * added at 20180408
*/
string processMigrantParameters(vector<string> args){
    string migrant;
    if(args.size() == 1){
    	cerr << "Error: -migrant option must accepts 1 argument but " << args.size() - 1 << " were provided." << "\n"
    	        << "Exiting..." << "\n";
    	exit(1);
    }
    else if(args.size() == 2){
    	migrant = args[1];
    	cout << "Begin remove migrant ion: " << migrant << endl;
    }
    else{
        cerr << "Error: -migrant option accepts at most 1 argument but " << args.size() - 1 << " were provided." << "\n"
        << "Exiting..." << "\n";
        exit(1);
    }
    return migrant;
