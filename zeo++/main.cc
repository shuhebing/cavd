...
//    exit(0);
  ...
  if(!error) {
    ...
    if(!error) {
      ...
      // added at 20180408
      bool useMigrant = false;
      const char* migrant;
      char* treatedFilename;
	...
	//added at 20180408
        ...
        else if(commands[i][0].compare("-nomigrant") == 0){
          useMigrant = false;
        }
      }
      ...
      if(strcmp(extension, "cuc") == 0){
        ...
        if(!readOBCSSRFile(filename, &atmnet, radial)) error=true;
    	  ...
    	  //remove migrant ion
    	  if(useMigrant){
    		  const char* s = pretreatCifFilename(filename,migrant).c_str();
    		  char* treatedFilename;
    		  strcpy(treatedFilename,s);
    		  cout << treatedFilename << endl;
    		  if(!readCIFFile(treatedFilename, &atmnet, radial)) error=true;
			}else
  ...
  //      exit(1);
      ...
      //	exit(1);
          ...
          //added at 20180408
			   ...
			   if(filename.empty()) {error=true; break;}
			   if(!writeToBI((char *)filename.data(), &atmnet, &vornet)) {error=true; break;}
          }
		  
		   //added at 20180603
		  else if(command[0].compare("-vasp") == 0){
        	   string filename =  processFilename(command, name, ".vasp", 0, 1);
			   string filename2 =  processFilename(command, name, "_orgin.vasp", 0, 1);
			   if(filename.empty()) {error=true; break;}
			   if(!writeToVasp((char *)filename2.data(), &atmnet, &vornet, true)) {error=true; break;}
			   if(!writeToVasp((char *)filename.data(), &atmnet, &vornet, true, 0.5, 1.0)) {error=true; break;}
          }
            ...
            string filename = processFilename(command, name, ".resex", 0, 1);
            if(filename.empty()) {error=true; break;}
            calculateFreeSphereParameters(&vornet, (char *)filename.data(), true);
            int *result;
//			   cout << throughVorNet(&vornet, (char *)filename.data(), 0.3) << endl;
...
//            calculateAverageGrid(&atmnet, filename_InputData, filename_Gaussian_cube, angstrom_to_bohr, useMass);
	      ...
	      if(visVisITflag) {
			...
			//Test code
			string filename2 = processFilename(command, name, ".zsa2", 3, 4);
			computeASA_new((char *)filename2.data(), &atmnet, highAccuracy, chan_radius, probe_radius, numSamples);
  ...
  //            exit(EXIT_FAILURE);
          ...
          else if(command[0].compare("-migrant") == 0 || command[0].compare("-nomigrant") == 0){
            // Preprocessed atomic masses
          }
