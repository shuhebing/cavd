"""
filename: resolve_symmetry.py

This file use to haddle symmetry in crystal. From .vasp file
get symmetrically independent atoms. 
...
import re
from cavd.netstorage import VoronoiNetwork,AtomNetwork
...
def Resolve_Symmetry_pym(filename):
    with zopen(filename, "rt") as f:
        contents = f.read()
    struc = Poscar.from_string(contents, False, read_velocities=False).structure
    positions = []
    lattice = struc.lattice.matrix
    print(lattice)
    for i in struc.sites:
        positions.append(i._fcoords)
    print(positions)
    print(len(positions))
    numbers = list(range(1,len(positions)+1,1))
    print(numbers)
    #numbers = struc.atomic_numbers
...
def get_Symmetry(atmt, voront):
   
    positions = poscar.coords
    #positions = []
    print(positions)
    lattice = atmt.lattice
    #for i in s2.sites:
    #    positions.append(i._fcoords)
    atomic_symbols = poscar.atomic_symbols
    print(atomic_symbols)
...
if __name__ == "__main__":
    Resolve_Symmetry("../../examples/icsd_16713.vasp")