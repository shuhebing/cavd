...
#自定义半径数据结构 
class Radius():
    def __init__(self, label, element, coord_num, radius):
        self._label = label
        self._element = element
        self._coord_num = coord_num
        self._radius = radius
    #列表labels, elements, coord_nums, radii必须具有相同的大小
    @staticmethod
    def get_radii_list(labels, elements, coord_nums, radii):
        radii_list = []
        if len(labels) != len(elements) != len(coord_nums) != len(radii):
            raise ValueError("labels, elements, coord_nums, radii must be have same length!")
        for i in range(len(labels)):
            radii_list.append(Radius(labels[i], elements[i], coord_nums[i], radii[i]).as_dict())
        return radii_list
...
#     cn_list = list(zip(el, cn))
#     radii_list = list(zip(el, radii))
#     radii_cn_list = list(zip(el, cn, radii))
#     label_cn_radii_list = list(zip(labels, cn, radii))

    #按label计算出配位数、半径   
#    cn_list = list(zip(el2, cn2))
#    radii_list = list(zip(el2, radii2))
#    radii_cn_list = list(zip(el2, cn2, radii2))
